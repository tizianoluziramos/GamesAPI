...
class RecommenderController {
  ...
  public async recommend(req: Request, res: Response) {
    try {
      ...
      const rawGames = await RecommenderRepository.getAllGames();
      ...
      if (RecommenderController.featuresCache.key === cacheKey && Date.now() - RecommenderController.featuresCache.ts < CACHE_TTL_MS && RecommenderController.featuresCache.payload) {
        const p = RecommenderController.featuresCache.payload!;
        ...
        if (wantPrecomputed && typeof (RecommenderRepository as any).getPrecomputedEmbeddings === "function") {
          // repository may return embeddings aligned with validatedGames order
          const emb = await (RecommenderRepository as any).getPrecomputedEmbeddings();
        ...
        RecommenderController.featuresCache = {
          ts: Date.now(),
          key: cacheKey,
          payload: {
            combinedVectors,
      ...
      const annAvailable = typeof (RecommenderRepository as any).searchAnn === "function" || (RecommenderController.featuresCache.payload && typeof (RecommenderController.featuresCache.payload as any).annSearch === "function");
      ...
      const topKSimilar = (aIdx: number, k: number, excludeSelf = true) => {
        ...
        if (annAvailable) {
          try {
            ...
            if (typeof (RecommenderRepository as any).searchAnn === "function") {
              const qvec = combinedVectorsNorm[aIdx];
              // repository function expected to return [{ idx, score }] or index list
              const out = (RecommenderRepository as any).searchAnn(qvec, k + (excludeSelf ? 1 : 0));
            ...
            if (RecommenderController.featuresCache.payload && typeof (RecommenderController.featuresCache.payload as any).annSearch === "function") {
              const idxs = (RecommenderController.featuresCache.payload as any).annSearch(combinedVectorsNorm[aIdx], k + (excludeSelf ? 1 : 0));
